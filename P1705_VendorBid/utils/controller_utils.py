...
def get_nice_error_message(*fields, err_type: str, default: str) -> str:
    message_mappings = {
        ...
        "vat": {
            "string_pattern_mismatch": "TIN should be 16 digits long"
        }
    }
    for field in fields:
...
def render_registration_error_html(env: Environment, errors: List[dict]):
    ...
    template_name = 'P1705_VendorBid.supplier_registration_error'
...
def get_rfp_general_search_domain(env: Environment) -> List[Tuple[str, str, str]]:
    ...
    elif env.user.has_group('P1705_VendorBid.group_supplies_requester'):
